  ...
  <body>
    <nav class="navbar">
      ...
      <ul class="nav-links">
        ...
        <li><a href="{{ route('user.belanja') }}" class="nav-item active">Belanja</a></li>
        <li><a href="{{ route('user.riwayat') }}" class="nav-item">Riwayat Belanja</a></li>
        <li><a href="{{ route('user.pinjaman') }}" class="nav-item">Pinjaman</a></li>
      </ul>
      ...
      <div class="auth-button">
        ...
        <div class="user-logged-in" style="display: flex; align-items: center; gap: 15px;">
            ...
            {{-- TOMBOL KERANJANG --}}
            <a href="{{ route('user.cart') }}" class="btn-cart" style="position: relative; color: #2c3e50; background: white; padding: 8px; border-radius: 50%; display: flex; align-items: center; justify-content: center; text-decoration: none; border: 1px solid #ddd;">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <circle cx="9" cy="21" r="1"></circle>
                    <circle cx="20" cy="21" r="1"></circle>
                    <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path>
                </svg>
                {{-- Angka jumlah barang di keranjang (opsional jika session cart ada) --}}
    ...
    <div class="shop-wrapper">
      <div class="container">
        <header class="shop-header">
          ...
          {{-- SEARCH BAR (DI ATAS FILTER, HANYA UNTUK LOGIN) --}}
          @auth
          ...
          {{-- TOMBOL FILTER (DI BAWAH SEARCH BAR) --}}
        ...
        <div class="product-grid">
          ...
          @else
            @foreach($products as $product)
              ...
              {{-- Sisipkan data-category di dalam div product-card --}}
              <div class="product-card{{ $isHidden ? ' hidden-item' : '' }}" data-category="{{ strtolower($product->category ?? 'lainnya') }}">
                <span class="badge-category">{{ $product->category ?? 'Produk' }}</span>
                <div class="product-img-wrapper">
                  <img src="{{ $productImage }}" alt="{{ $product->name ?? 'Produk' }}" />
                </div>
                <div class="product-info">
                  <h3>{{ $product->name ?? 'Produk' }}</h3>
                  <p class="product-desc">{{ $product->description ?? 'Deskripsi belum tersedia.' }}</p>
                  <p class="product-price">Rp. {{ number_format($productPrice, 0, ',', '.') }}</p>

                  {{-- GANTI BAGIAN FORM SEBELUMNYA DENGAN INI --}}
                  @auth
                    <a href="#" class="btn-add-cart open-cart-modal" 
                       data-id="{{ $product->id }}"
                       data-name="{{ $product->name ?? 'Produk' }}"
                       data-price="{{ $productPrice }}"
                       data-image="{{ $productImage }}"
                       data-url="{{ route('user.cart.add', $product->id) }}">
                      Masukan Keranjang
                    </a>
                  @endauth
        ...
        @if(count($products) > $visibleLimit)
        ...
        <div class="modal-content" style="max-width: 350px; text-align: center; padding: 25px;">
            ...
            <form id="cartForm" method="POST" action="">
                ...
                <div style="display: flex; justify-content: center; align-items: center; gap: 15px; margin: 20px 0; background: #f4f6f9; padding: 10px; border-radius: 50px;">
                    ...
                    {{-- Input QTY ini yang akan dikirim ke Laravel --}}
    ...
    <script src="{{ asset('user/js/belanja.js') }}"></script>
  </body>
</html>
