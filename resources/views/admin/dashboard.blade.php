...
@section('title', 'Dashboard Kasir | Admin')
...
<div class="cashier-page">
    ...
    <div class="stats">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value">{{ $summary['paid'] ?? 0 }}</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Potong Gaji</div>
            <div class="stat-value">{{ $summary['salary_cut'] ?? 0 }}</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Belum Dibayar</div>
            <div class="stat-value">{{ $summary['unpaid'] ?? 0 }}</div>
        </div>
    </div>
    ...
    <div class="grid">
        <div class="card">
            <div class="toolbar">
                ...
                <form method="GET" action="{{ route('admin.dashboard') }}" class="order-filter-form" id="orderFilterForm">
                    <select class="field-control field-select" name="payment_status">
                        <option value="ALL" {{ ($filterValues['payment_status'] ?? 'ALL') === 'ALL' ? 'selected' : '' }}>Semua Status</option>
                        <option value="TELAH_DIBAYAR" {{ ($filterValues['payment_status'] ?? 'ALL') === 'TELAH_DIBAYAR' ? 'selected' : '' }}>Telah Dibayar</option>
                        <option value="POTONG_GAJI" {{ ($filterValues['payment_status'] ?? 'ALL') === 'POTONG_GAJI' ? 'selected' : '' }}>Potong Gaji</option>
                        <option value="BELUM_DIBAYAR" {{ ($filterValues['payment_status'] ?? 'ALL') === 'BELUM_DIBAYAR' ? 'selected' : '' }}>Belum Dibayar</option>
                    </select>
                    ...
                    <input class="field-control"
                           ...
                           name="exact_date"
                           id="filterExactDate"
                           value="{{ (string) ($filterValues['exact_date'] ?? '') }}">
            ...
            <p class="active-filter">Periode Aktif: {{ $activePeriodLabel }}</p>
                ...
                <table class="orders-table">
                    ...
                    <tbody>
                        @forelse($orders as $order)
                            ...
                            @endphp
                            <tr>
                                <td>#{{ str_pad((string) $order->id, 5, '0', STR_PAD_LEFT) }}</td>
                                ...
                                <td>
                                    <form class="action-form"
                                            method="POST"
                                            action="{{ route('admin.dashboard.order.payment-status', $order->id) }}">
                                        @csrf
                                        <select class="field-control field-select" name="payment_status">
                                            <option value="TELAH_DIBAYAR" {{ $order->payment_status === 'TELAH_DIBAYAR' ? 'selected' : '' }}>Telah Dibayar</option>
                                            <option value="POTONG_GAJI" {{ $order->payment_status === 'POTONG_GAJI' ? 'selected' : '' }}>Potong Gaji</option>
        ...
        <div class="card">
            ...
            <form class="manual-form"
                  method="POST"
                  action="{{ route('admin.dashboard.order.manual') }}"
                  id="manualOrderForm">
                @csrf

                <input type="hidden" name="items_payload" id="manualItemsPayload" value="{{ old('items_payload', '') }}">

                <div class="manual-search-wrap">
                    <label>Customer</label>
                    <input type="hidden" name="user_id" id="manualCustomerId" value="{{ old('user_id', '') }}">
                    <input class="field-control"
                           type="text"
                ...
                <div>
                    ...
                    <select class="field-control field-select" name="metode" id="manualMetode" required>
                        ...
                        <option value="Potong Gaji" {{ old('metode') === 'Potong Gaji' ? 'selected' : '' }}>Potong Gaji</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select class="field-control field-select" name="payment_status" id="manualStatus" required>
                        <option value="TELAH_DIBAYAR" {{ old('payment_status', 'TELAH_DIBAYAR') === 'TELAH_DIBAYAR' ? 'selected' : '' }}>Telah Dibayar</option>
                        <option value="POTONG_GAJI" {{ old('payment_status') === 'POTONG_GAJI' ? 'selected' : '' }}>Potong Gaji</option>
                        <option value="BELUM_DIBAYAR" {{ old('payment_status') === 'BELUM_DIBAYAR' ? 'selected' : '' }}>Belum Dibayar</option>
                    </select>
                </div>
